"""Modèle de prédiction de la rétention (tâche 31 — Amina).
...
# --- Alias `train()` / `evaluate()` / `predict()` (interface demandée par la
# fiche de tâche 31) — enveloppent les fonctions ci-dessus sans dupliquer la
# logique, pour un vocabulaire cohérent avec le reste de l'équipe. ---

def train(X: pd.DataFrame, y: pd.Series, name: str | None = None):
    """Entraîne le modèle final. Si `name` est omis, sélectionne le meilleur en LOO-CV."""
    if name is None:
        name = best_model_name(evaluate_models(X, y))
    return fit_final(X, y, name)


def evaluate(X: pd.DataFrame, y: pd.Series) -> dict:
    """Alias de `evaluate_models` (LOO-CV, MAE/R² par modèle + baseline)."""
    return evaluate_models(X, y)


def predict(model, X: pd.DataFrame) -> np.ndarray:
    """Prédit la rétention (0..1) pour les lignes de `X` avec un modèle entraîné."""
